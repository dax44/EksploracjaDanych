...
I tak u podstaw metod bazujących na prostych drzewach decyzyjnych stał pomysł, że skoro jedno drzewo nie ma wystrczających własności predykcyjnych, to może zastosowanie wielu drzew połączonych w pewien sposób poprawi je. Tak powstały metody *bagging*, *random forest* i *boosting*^[hyba tylko dla drógiej metody istniej dobre polskie tłumaczenie nazwy - las losowy]. Należy zaznaczyć, że metody zanjdują swoje zastosowanie również w innych modelach nadzorowanego ucznienia maszynowego.
...
Implementacja R-owa metody bagging znajduje się w pakiecie **ipred**, a funkcja do budowy modelu nazywa się `bagging` [@R-ipred]. Można również stosować funkcję `randomForest` pakietu **randomForest** [@R-las] - powody takiego działania wyjaśnią się w podrodziale [Lasy losowe].
...
2. Dla $b=1,2,\ldots, B$ powtarzaj:
   ...
   b) zakutalizuj drzewo do nowej "skurczonej" wersji 
   \begin{equation}
